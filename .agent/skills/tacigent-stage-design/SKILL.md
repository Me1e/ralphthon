...
- 정보 구조, flow, screen/state, responsive behavior를 **구현 가능한 형태**로 남긴다
...
### Step 1: Method Framing (Design-Specific)
...
- system anchor가 강하고 risk가 낮거나 single-direction refinement가 충분히 설명 가능하면 **3 또는 single-direction path**로 downshift
...
third critique는 public-facing first impression risk가 높거나, direction 간 contradiction이 남거나, brand/legal sensitivity가 커질 때만 escalation.
...
- breakpoint density budget과 interaction state completeness가 충족되는가?
</MANDATORY>
...
위 Ready-For-Dev Gate를 통과했을 때 canonicalization 가능.
