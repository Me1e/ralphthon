...
### Step 1: Method Framing + Meta-Research (Solution-Specific)
...
- `keyWorkflow` — 핵심 사용 흐름 (복수 가능)
- `whyThisWins` — 기존 대안 대비 왜 이것인가
- `fullFeatureSet` — 이 solution을 온전히 구현하기 위한 전체 기능 목록
- `screenInventory` — 필요한 화면/route 목록 (최소 3개 이상)
- `dataModel` — 필요한 데이터 모델 윤곽

<MINIMUM-PRODUCT-SCOPE>
solution의 fullFeatureSet은 아래 최소 기준을 충족해야 한다:
- **최소 3개 이상의 distinct screen/route** (1-screen 앱 금지)
- **최소 1개의 CRUD flow** (생성/조회/수정/삭제 중 3개 이상)
- **인증이 필요하면 로그인/가입 + 인증된 기능** 포함
- **solution의 핵심 약속(primaryUserPromise)을 실현하는 전체 workflow** — 한 단계가 아니라 end-to-end
...
- constraint screen이 빠르게 feasible set을 좁히거나 viable candidate가 거의 하나로 수렴하면 **5로 downshift** (5 미만 금지)
...
solution.md 작성 후, 5회의 제1원칙 리뷰를 수행한다. **Solution-specific 리뷰 기준:**

1. **coreMechanism이 진짜로 다른가?** — 기존 대안의 변형일 뿐이면 mechanism 자체를 재정의
2. **fullFeatureSet이 solution을 온전히 구현하는가?** — 핵심 약속을 실현하려면 빠진 기능이 없는지 확인. 1-screen은 금지
3. **사용자 약속(primaryUserPromise)이 검증 가능한가?** — 모호한 약속이면 구체적 metric으로 교체
4. **problem과의 정합성** — solution이 problem을 직접 풀고 있는지, 딴 문제를 풀고 있지 않은지
5. **end-to-end workflow가 완전한가?** — 사용자가 처음 방문해서 핵심 가치를 경험하기까지 모든 단계가 정의되었는지
...
- mvpScope와 buildConstraints가 비어 있지 않고
- **5회 산출물 리뷰가 완료되었을 때 (review-1~5.md)**
