...
### Step 1: Method Framing + Meta-Research (Problem-Specific)
...
- upstream frame이 매우 좁거나 viable problem cluster가 빠르게 수렴하면 **5로 downshift** (5 미만 금지)
- 각 branch에서:
...
problem.md 작성 후, 5회의 제1원칙 리뷰를 수행한다. **Problem-specific 리뷰 기준:**

1. **선택한 problem이 정말 아파야 하는 문제인가?** — "있으면 좋겠다" 수준이면 삭제하고 다시 선택
2. **evidence가 problem을 직접 뒷받침하는가?** — 간접적 근거만 있으면 보강 또는 problem 자체를 수정
3. **problem statement에 solution bias가 섞여 있지 않은가?** — "X가 없다"는 problem이 아니라 solution
4. **target user가 충분히 좁은가?** — "모든 개발자"는 너무 넓음
5. **failure moment가 구체적인가?** — 시나리오가 아니라 실제 관찰 가능한 순간이어야 함
...
- **5회 산출물 리뷰가 완료되었고 (review-1~5.md)**
- readiness가 명시되었을 때
